...
"""
Dataset generator for ICLR paper acceptance prediction with arXiv search.
...
{
    "data_source": "searchR1_arxiv_iclr",
...
DEFAULT_BINARY_SYSTEM_CONTENT = """You are an expert academic reviewer for the ICLR conference, tasked with evaluating research papers. Your task is to predict whether a paper will be accepted or rejected.
...
DEFAULT_CITATION_SYSTEM_CONTENT = """You are an expert academic tasked with predicting the citation rank of ICLR papers. We compare 
 - Note: ICLR generally has a ~30% acceptance rate
...
def process_single_row(
    ...
    data_source_tagged = "searchR1_arxiv_iclr"
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert ShareGPT-format ICLR data to training parquet for search_arxiv environment."
    )
    parser.add_argument(
        "--input_json",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/scratch/gpfs/ZHUANGL/sk7524/SkyRL/skyrl-train/data/arxiv_iclr",
    ...
    # Set content
    system_content = args.system_content if args.system_content else DEFAULT_BINARY_SYSTEM_CONTENT
    user_content_prefix = ""
