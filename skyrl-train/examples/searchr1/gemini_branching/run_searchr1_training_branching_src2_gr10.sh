#!/bin/bash
#SBATCH --job-name=searchr1_gemini_branch_src2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "==========================================="
echo "SearchR1 GEMINI Branched GRPO Training"
echo "Model: Qwen/Qwen3-4B-Instruct-2507"
echo "Backend: vLLM"
echo "GPUs: 4"
echo "Branching: strategy=gemini, src_trajectories=2, n_samples_per_prompt=10"
...
EXPORT_PATH="/scratch/gpfs/ZHUANGL/sk7524/SkyRL/skyrl-train/exports/searchr1_gemini_branched/${SLURM_JOB_ID}"
...
python -m skyrl_train.entrypoints.main_base \
  ...
  trainer.run_name="searchr1_gemini_branched_grpo_src2_gr10_${SLURM_JOB_ID}" \
  ...
  generator.branching.src_trajectories=2 \
  generator.branching.num_branches=2 \
  generator.branching.strategy="gemini" \
  generator.branching.gemini.model="gemini-2.5-flash" \
  generator.branching.gemini.timeout=30.0 \
