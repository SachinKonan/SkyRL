...
_TOOL_USE_APPENDIX = (
    "\n\n---\n\n"
    "### Tool Use\n\n"
    "Inside your reasoning you have two tools. Each tool call must appear on its "
    "own after the closing `</think>` tag and before you emit another `<think>` block.\n\n"
    "1. Semantic search over arXiv:\n"
    "     <ssearch>\n"
    "     first query\n"
    "     second query\n"
    "     ...\n"
    "     </ssearch>\n"
    "   The body is one query per line (a single-line body is one query). "
    "Results come back between `<information>` and `</information>` with "
    "`## Query k: \"...\"` section headers so you can see which results came "
    "from which query.\n\n"
    "2. Python execution:\n"
    "     <python>\n"
    "     print(1 + 1)\n"
    "     </python>\n"
    "   Stdout (or the error message) comes back between `<information>` and "
    "`</information>`. Use this for quick numeric sanity-checks on the paper's "
    "claims, statistical tests on reported numbers, etc.\n\n"
    "Use retrieval to calibrate against prior work; use Python to check "
    "quantitative claims."
)
